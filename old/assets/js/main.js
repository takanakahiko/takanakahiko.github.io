  ...
  $window.on('load', function() {
    skel
      ...
      .on('+desktop', function() {
        const $body = $('body')
        const $main = $('#main')
        const $panels = $main.find('.panel')
        const $hbw = $('html,body,window')
        const $footer = $('#footer')
        const $wrapper = $('#wrapper')
        const $nav = $('#nav')
        const $nav_links = $nav.find('a')
        ...
        $body._resize = function() {
          const factor = ($window.width() * $window.height()) / (1440 * 900)
          $body.css(
            'font-size',
            Math.min(
              Math.max(
                Math.floor(factor * settings.sizeFactor),
                settings.sizeMin
              ),
              settings.sizeMax
            ) + 'pt'
        ...
        $body._reposition = function() {
          if (
            ...
            $wrapper.css(
              ...
              Math.max(
                ($window.height() -
                  (panels[activePanelId].outerHeight() +
                    $footer.outerHeight())) /
                  2 -
                  $nav.height(),
                30
            ...
            $wrapper.css(
              'padding-top',
              ($window.height() - panels[firstPanelId].height()) / 2 -
                $nav.height() +
                'px'
            )
        }
        ...
        $panels.each(function(i) {
          ...
          t._activate = function(instant) {
            ...
            if (isLocked || activePanelId == id) return false
            ...
            if (x > $window.height()) $wrapper.addClass('tall')
            else $wrapper.removeClass('tall')

            // Fade out active panel.
            $footer.fadeTo(settings.fadeSpeed, 0.0001)
            panels[activePanelId].fadeOut(
              instant ? 0 : settings.fadeSpeed,
              function() {
                // Set new active.
                activePanelId = id
        ...
        $window.resize(function() {
          if (!isLocked) $body._resize()
        })

        $window.on('orientationchange', function() {
          if (!isLocked) $body._reposition()
        })
        ...
        if (hash && hash in panels) panels[hash]._activate(true)
